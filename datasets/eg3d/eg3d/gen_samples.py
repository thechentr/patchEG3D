...
def generate_images(
    ...
    G.rendering_kwargs['ray_start'] = G.rendering_kwargs['ray_end'] = 'auto'
    G.rendering_kwargs['avg_camera_radius'] = 5.0
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        imgs = []
        angle_y = .1
        for angle_y, angle_p in [(angle_y, -.8), (angle_y, -.4), (angle_y, .0), (angle_y, .4), (angle_y, -.8)]:
            cam_pivot = torch.tensor(G.rendering_kwargs.get('avg_camera_pivot', [0, 0, 0]), device=device)
            cam_radius = G.rendering_kwargs.get('avg_camera_radius', 2.7)
            cam2world_pose = LookAtPoseSampler.sample(angle_y, np.pi/2 + angle_p, cam_pivot, radius=cam_radius, device=device)
